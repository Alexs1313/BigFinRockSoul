import {
  ...
  partyZoneAssets,
  partyZoneFishImages,
} from '../biggysouldt/partyZoneAssets';
...
import BigFinPillButton from '../biggyslcomponts/BigFinPillButton';
...
export default function BigFinPartyZoneScreen() {
  ...
  const scoreBadgeMarginT = 20;
  const scoreTextSize = 20;
  const modalCardWidth = 88;
  const modalPad = 18;
  const modalPadT = 24;
  const modalTitleSize = 20;
  const modalPriceSize = 22;
  const modalButtonsGap = 16;
  const modalButtonsMarginT = 10;
  ...
  const bigFinLoadPartyZone = useCallback(async () => {
    try {
      ...
      if (storedScores === null) {
        ...
        setBigFinScore(DEFAULT_FLAMINGO_BALANCE);
      } else {
        const parsedScore = parseInt(storedScores, 10);
        setBigFinScore(bigFinSafeNumber(parsedScore));
      }

      // All 7 fish are always unlocked (no level progress required)
      const allFishUnlocked = 7;
      const storedUnlocked = await AsyncStorage.getItem(
        bigFinStorageKeys.unlockedFish,
      );
      const parsedUnlocked = bigFinSafeNumber(
        storedUnlocked ? parseInt(storedUnlocked, 10) : allFishUnlocked,
      );
      const normalizedUnlocked = Math.max(
        1,
        Math.min(7, parsedUnlocked || allFishUnlocked),
      );

      if (storedUnlocked === null || normalizedUnlocked !== allFishUnlocked) {
        await AsyncStorage.setItem(
          bigFinStorageKeys.unlockedFish,
          String(allFishUnlocked),
        );
      }
      setBigFinUnlockedFishCount(allFishUnlocked);
  ...
  return (
    <FinBack
      source={require('../assets/finImages/danceBg.png')}
      style={stules.cont}
      resizeMode="cover"
    >
      <ScrollView
