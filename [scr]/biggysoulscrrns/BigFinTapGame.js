import React, { useState, useEffect, useRef, useCallback } from 'react';
import { Dimensions, StyleSheet, Share } from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { useFocusEffect, useNavigation } from '@react-navigation/native';
import { useFinStore } from '../biggysoulstrg/bigFinCntxt';
import TapGameLoadingScreen from '../biggyslcomponts/TapGameLoadingScreen';
import TapGameStartScreen from '../biggyslcomponts/TapGameStartScreen';
import TapGamePlayScreen from '../biggyslcomponts/TapGamePlayScreen';
import TapGameResultScreen from '../biggyslcomponts/TapGameResultScreen';
import Orientation from 'react-native-orientation-locker';

const tapStorageKeys = {
  totalScore: 'flamingo_total_score',
};

const DEFAULT_FLAMINGO_BALANCE = 60;

export function BigFinTapGame() {
  const navigation = useNavigation();
  const { finSoundEnabled: soundEnabled, setFinSoundEnabled } = useFinStore();

  const [gamePhase, setGamePhase] = useState('start');
  const [score, setScore] = useState(0);
  const [timeLeft, setTimeLeft] = useState(30);
  const [pointsBalance, setPointsBalance] = useState(0);

  const [bubbles, setBubbles] = useState([]);
  const [isRunning, setIsRunning] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [isBooting, setIsBooting] = useState(true);

  const { width: screenWidth, height: screenHeight } = Dimensions.get('window');
  const scoreRef = useRef(0);
  const bubbleIdRef = useRef(0);

  const timerRef = useRef(null);
  const spawnRef = useRef(null);
  const moveRef = useRef(null);

  useFocusEffect(() => {
    getInitState();
    getSoundSetting();
  });

  useFocusEffect(
    useCallback(() => {
      Orientation.lockToPortrait();

      return () => {
        Orientation.unlockAllOrientations();
      };
    }, []),
  );

  const getSoundSetting = async () => {
    try {
      const savedSoundVale = await AsyncStorage.getItem('toggleSound');

      const parsedJSON = JSON.parse(savedSoundVale);

      setFinSoundEnabled(parsedJSON);
    } catch (e) {
      console.error('play soun err', e);
    }
  };

  useEffect(() => {
    if (isRunning && !isPaused) {
      timerRef.current = setInterval(() => {
        setTimeLeft(prev => {
          if (prev <= 1) {
            finishRound();
            return 0;
          }
          return prev - 1;
        });
      }, 1000);

      spawnRef.current = setInterval(() => {
        spawnBubble();
      }, 800);

      moveRef.current = setInterval(() => {
        setBubbles(prevDrops =>
          prevDrops
            .map(drop => ({ ...drop, y: drop.y + 14 }))
            .filter(drop => drop.y < screenHeight + 100),
        );
      }, 30);
    }

    return () => {
      if (timerRef.current) clearInterval(timerRef.current);
      if (spawnRef.current) clearInterval(spawnRef.current);
      if (moveRef.current) clearInterval(moveRef.current);
    };
  }, [isRunning, isPaused, screenHeight, spawnBubble]);

  const getInitState = async () => {
    try {
      const storedWallet = await AsyncStorage.getItem(
        tapStorageKeys.totalScore,
      );

      if (storedWallet === null) {
        await AsyncStorage.setItem(
          tapStorageKeys.totalScore,
          String(DEFAULT_FLAMINGO_BALANCE),
        );
        setPointsBalance(DEFAULT_FLAMINGO_BALANCE);
      } else {
        setPointsBalance(parseInt(storedWallet, 10));
      }
    } catch (error) {
      console.warn('game loading error', error);
    } finally {
      setIsBooting(false);
    }
  };

  const hasBubbleOverlap = (fresh, existing) => {
    const minGap = 100;
    for (let one of existing) {
      const dist = Math.sqrt(
        Math.pow(fresh.x - one.x, 2) + Math.pow(fresh.y - one.y, 2),
      );
      if (dist < minGap) return true;
    }
    return false;
  };

  const spawnBubble = useCallback(() => {
    console.log('spawnBubble');
    setBubbles(prevDrops => {
      const isFlamingo = Math.random() > 0.4;
      const size = 70 + Math.random() * 10;

      const maxTries = 10;
      let tries = 0;
      let fresh;

      do {
        const x = Math.random() * (screenWidth - size - 40) + 20;
        const y = -size - Math.random() * 200;
        fresh = {
          id: bubbleIdRef.current,
          x,
          y,
          size,
          isFlamingo,
        };
        tries++;
      } while (hasBubbleOverlap(fresh, prevDrops) && tries < maxTries);

      if (tries < maxTries) {
        bubbleIdRef.current += 1;
        return [...prevDrops, fresh];
      }

      return prevDrops;
    });
  }, [screenWidth]);

  useEffect(() => {
    if (gamePhase === 'start') {
      AsyncStorage.getItem(tapStorageKeys.totalScore).then(v => {
        setPointsBalance(v ? Number(v) : DEFAULT_FLAMINGO_BALANCE);
      });
    }
  }, [gamePhase]);

  const popBubble = item => {
    if (item.isFlamingo) {
      scoreRef.current += 1;
      setScore(scoreRef.current);
    }
    setBubbles(prevDrops => prevDrops.filter(b => b.id !== item.id));
  };

  const startRound = () => {
    setScore(0);
    scoreRef.current = 0;
    setTimeLeft(30);
    setBubbles([]);
    bubbleIdRef.current = 0;
    setIsRunning(true);
    setIsPaused(false);
    setGamePhase('game');
  };

  const formatTime = seconds => {
    const m = Math.floor(seconds / 60);
    const s = seconds % 60;
    return `${m.toString().padStart(2, '0')}:${s.toString().padStart(2, '0')}`;
  };

  const finishRound = async () => {
    setIsRunning(false);

    const earned = scoreRef.current;

    try {
      const storedWallet = await AsyncStorage.getItem(
        tapStorageKeys.totalScore,
      );
      const current = storedWallet
        ? Number(storedWallet)
        : DEFAULT_FLAMINGO_BALANCE;

      const nextTotal = current + earned;

      await AsyncStorage.setItem(tapStorageKeys.totalScore, String(nextTotal));
      setPointsBalance(nextTotal);
    } catch (e) {
      console.log('game save error:', e);
    }

    setGamePhase('result');
  };

  const togglePause = () => setIsPaused(p => !p);

  const setSoundEnabled = async next => {
    try {
      await AsyncStorage.setItem('toggleSound', JSON.stringify(next));
      setFinSoundEnabled(next);
    } catch (e) {
      console.log('Error toggle sound', e);
    }
  };

  const shareScore = () => {
    Share.share({ message: `Points earned +${score}` })
      .then(r => console.log(r))
      .catch(e => console.log(e));
  };

  if (isBooting) {
    return <TapGameLoadingScreen styles={tapStyles} />;
  }

  if (gamePhase === 'start') {
    return (
      <TapGameStartScreen
        styles={tapStyles}
        navigation={navigation}
        wallet={pointsBalance}
        onBeginRun={startRound}
      />
    );
  }

  if (gamePhase === 'game') {
    return (
      <TapGamePlayScreen
        styles={tapStyles}
        navigation={navigation}
        secondsLeft={timeLeft}
        drops={bubbles}
        onPopBubble={popBubble}
        onPause={togglePause}
        onRetry={() => {
          setIsPaused(false);
          setScore(0);
          scoreRef.current = 0;
          setTimeLeft(30);
          setBubbles([]);
          bubbleIdRef.current = 0;
        }}
        paused={isPaused}
        soundEnabled={soundEnabled}
        onToggleSound={setSoundEnabled}
        formatTimer={formatTime}
        onExitToMenu={() => {
          setIsRunning(false);
          setIsPaused(false);
          navigation.goBack();
        }}
      />
    );
  }

  if (gamePhase === 'result') {
    return (
      <TapGameResultScreen
        styles={tapStyles}
        score={score}
        wallet={pointsBalance}
        onShare={shareScore}
        onPartyZone={() => navigation.navigate('BigFinPartyZoneScreen')}
        onExit={() => setGamePhase('start')}
      />
    );
  }

  return null;
}

const tapStyles = StyleSheet.create({
  tapScreen: { flex: 1 },
  tapSafeArea: { flex: 1 },
  tapLoadingBox: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
  },
  tapLoadingLabel: { color: '#fff', fontWeight: '600' },
  tapHeaderShell: { width: '90%', alignSelf: 'center', zIndex: 1 },
  tapHeaderContent: { alignItems: 'center', justifyContent: 'center' },
  tapBackButton: { position: 'absolute' },
  tapScreenTitle: { fontWeight: '900', color: '#FDEB57' },
  tapHeaderStars: { position: 'absolute', width: '100%', zIndex: -1 },
  tapPointsBadge: {
    width: 132,
    height: 34,
    justifyContent: 'center',
    alignItems: 'center',
    alignSelf: 'center',
  },
  tapPointsBadgePosition: { marginTop: 8 },
  tapPointsFlamIcon: {
    position: 'absolute',
    left: 14,
    width: 20,
    height: 20,
  },
  tapIntroCharacter: {
    width: 160,
    height: 180,
    marginBottom: 16,
  },
  tapIntroTextBlock: {
    alignItems: 'center',
    marginBottom: 32,
    paddingHorizontal: 24,
  },
  tapIntroTitle: {
    color: '#fff',
    fontSize: 18,
    fontWeight: '700',
    textAlign: 'center',
    marginBottom: 8,
  },
  tapIntroSubtitle: {
    color: '#fff',
    fontSize: 16,
    textAlign: 'center',
    fontWeight: '500',
    lineHeight: 24,
  },
  tapCooldownBlock: { alignItems: 'center' },
  tapPointsValue: {
    color: '#FDEB57',
    fontSize: 20,
    textAlign: 'center',
    fontWeight: '900',
    left: 10,
  },
  tapContentCenter: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  tapHintText: {
    color: '#fff',
    fontSize: 16,
    marginBottom: 40,
    textAlign: 'center',
    fontWeight: '500',
  },
  tapCooldownLabel: {
    color: '#fff',
    fontSize: 18,
    marginBottom: 20,
    textAlign: 'center',
  },
  tapCooldownValue: { color: '#FDEB57', fontWeight: '700', fontSize: 24 },
  tapPrimaryButton: {
    width: 258,
    height: 71,
    borderRadius: 16,
    alignItems: 'center',
    justifyContent: 'center',
    borderWidth: 1,
    borderColor: '#fff',
  },
  tapPrimaryButtonLabel: { color: '#fff', fontWeight: '900', fontSize: 24 },
  tapGlowDot: {
    position: 'absolute',
    bottom: 40,
    left: 40,
    width: 60,
    height: 60,
    borderRadius: 30,
    backgroundColor: 'rgba(128, 90, 213, 0.3)',
    borderWidth: 2,
    borderColor: 'rgba(252, 230, 253, 0.2)',
  },
  tapButtonStars: { position: 'absolute', right: 10, bottom: 5 },
  tapToolbarRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 16,
    zIndex: 1,
  },
  tapTimerBadge: {
    backgroundColor: '#1C0234',
    paddingHorizontal: 24,
    paddingVertical: 10,
    borderRadius: 25,
    borderWidth: 1,
    borderColor: '#691D7A',
    minWidth: 130,
    justifyContent: 'center',
    alignItems: 'center',
  },
  tapTimerValue: { color: '#FDEB57', fontSize: 22, fontWeight: '900' },
  tapPauseButton: {
    backgroundColor: '#1C0234',
    width: 50,
    height: 50,
    borderRadius: 25,
    alignItems: 'center',
    justifyContent: 'center',
    borderWidth: 1,
    borderColor: '#691D7A',
  },
  tapBubblesArea: { flex: 1, position: 'relative' },
  tapBubbleWrap: {
    position: 'absolute',
    alignItems: 'center',
    justifyContent: 'center',
  },
  tapBubbleBack: { width: '100%', height: '100%', position: 'absolute' },
  tapBubbleFront: { width: '60%', height: '60%' },
  tapPauseOverlay: {
    position: 'absolute',
    top: 0,
    left: 0,
    right: 0,
    bottom: 0,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'rgba(0, 0, 0, 0.15)',
    zIndex: 2,
  },
  tapPauseCard: {
    width: '70%',
    borderRadius: 16,
    borderWidth: 1.5,
    borderColor: '#F9CDF9',
  },
  tapPauseCardContent: {
    padding: 15,
    alignItems: 'center',
  },
  tapPauseActionBtn: {
    backgroundColor: 'rgba(16, 6, 61, 0.9)',
    width: '100%',
    height: 40,
    borderRadius: 50,
    marginVertical: 8,
    justifyContent: 'center',
    alignItems: 'center',
  },
  tapPauseActionLabel: {
    color: '#fff',
    fontSize: 18,
    fontWeight: '500',
    fontStyle: 'italic',
  },
  tapSoundButton: {
    marginTop: 16,
    backgroundColor: 'rgba(16, 6, 61, 0.9)',
    width: 50,
    height: 50,
    borderRadius: 25,
    alignItems: 'center',
    justifyContent: 'center',
  },
  tapResultSafeArea: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
    paddingHorizontal: 24,
  },
  tapResultTopImage: { marginBottom: 14 },
  tapResultNiceTapping: {
    fontSize: 28,
    fontWeight: '900',
    color: '#FDEB57',
    textAlign: 'center',
    marginBottom: 8,
    textShadowColor: '#fff',
    textShadowOffset: { width: 0, height: 1 },
    textShadowRadius: 2,
  },
  tapResultLine: {
    color: '#FDE6D9',
    fontSize: 16,
    textAlign: 'center',
    marginVertical: 2,
    fontWeight: '500',
  },
  tapResultReceivedRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    marginTop: 24,
    marginBottom: 32,
    gap: 8,
  },
  tapResultReceivedLabel: {
    fontSize: 24,
    fontWeight: '900',
    color: '#FDEB57',
    textShadowColor: '#fff',
    textShadowOffset: { width: 0, height: 1 },
    textShadowRadius: 2,
  },
  tapResultFlamIcon: { width: 28, height: 28 },
  tapEarnedLabel: {
    fontSize: 24,
    fontWeight: '900',
    color: '#fff',
    marginTop: 30,
    marginBottom: 8,
  },
  tapTotalLabel: {
    fontSize: 24,
    fontWeight: '900',
    color: '#FDEB57',
    marginBottom: 40,
  },
  tapResultActions: { gap: 12 },
  tapResultScrollContent: {
    flexGrow: 1,
    paddingVertical: 40,
    paddingBottom: 120,
  },
  tapResultButton: { alignItems: 'center', justifyContent: 'center' },
  tapResultButtonSize: {
    minWidth: 216,
    height: 40,
    borderRadius: 50,
  },
  tapResultButtonLabel: {
    color: '#10063D',
    fontSize: 20,
    fontWeight: '600',
    fontStyle: 'italic',
  },
});
